...
Market Basket Analysis is a very helpful technique commonly used by retailers to uncover associations between items. \newline
The idea is to define certain (probability) rules and on that basis identify if products (items) occur together in a transaction (basket). It's a very intuitive analysis based on 3 key measures: Support, Lift and Confidence (4th measure, Expected Confidence, can be also considered) and therefore is easy understand and outcome is easy to explain. \newline
...
Let's upload required packages and data with customer orders in required format.
...
market_order <- read.transactions("data_basket/basket_data.csv",
                                  format = "single",
                                  sep = ",",
                                  header = TRUE,
